...
pdf("golebiewski_problem1.pdf",height=20,width=20)
heatmap.2(edata,
          main = "Bottomly et al. Clustered hw1", # heat map title
          notecol="black",      # change font color of cell labels to black
          density.info="none",  # turns off density plot inside color legend
          trace="none",         # turns off trace lines inside the heat map
          margins =c(12,9),     # widens margins around plot
...
pdf("golebiewski_problem2.pdf",height=10,width=10)
ggplot(PC) + geom_point(aes(x=V2, y=V3, col=as.factor(strain))) + labs(title="strain separation with combination of V2 and V3", x="V2", y="V3")
dev.off()
...
pdf("golebiewski_problem3.pdf",height=10,width=10)
plot(svd.out$u[,1], svd.out$u[,2],pch=20, main="top 2 left singular vectors", xlab="1st left singular vector", ylab="2nd left singular vector")
dev.off()
...
pdf("golebiewski_problem4.pdf", height=10, width=15)

top5 <- data.table(svd.out$u[,1:5]) 
melted_data <- melt(top5, measure.vars = colnames(top5), variable.name = "singular_vector")

ggplot(melted_data) + geom_violin(aes(x=as.factor(singular_vector), y=value), draw_quantiles = c(0.25, 0.5, 0.75)) + labs(title="top 5 left singular vectors (loadings)", x="Singular Vector", y="Value")
...
pdf("golebiewski_problem5.pdf",height=10,width=10)

# Set a seed for reproducible results
set.seed(1920)

kmeans_clusts <- kmeans(edata, centers = 5)
tsne_out <- Rtsne(edata, pca=FALSE, perplexity=60)
tsne_out <- data.table(tsne_out$Y)

ggplot(tsne_out) + geom_point(aes(x=V1, y=V2, col=as.factor(kmeans_clusts$cluster))) + labs(title=" t-SNE projection of k-means clustered genes", col="k-means clusters")
