...
pdf(file = "golebiewski_problem1.pdf", width=8, height=6)

batch_cancer <- table(pheno$batch, pheno$cancer) # cancer in batches
batch_outcome <- table(pheno$batch, pheno$outcome) # outcomes in batches
df_cancer <- as.data.frame.matrix(batch_cancer)
df_outcome <- as.data.frame.matrix(batch_outcome)


# COmbine the data into 1 table
combined_table <- cbind(df_cancer, df_outcome)
combined_table <- cbind(Batch = rownames(combined_table), combined_table)
grid.table(combined_table, rows = NULL)

dev.off()

# Ideally, the distribution of normal and cancer samples would be similar across batches. However, we can see that in our data. batch 1 consists of solely of cancer samples. Batch 3 on the other hand, has only normal cells and Batch 4 only biopsy cells. The amount of samples varies greatly too. That can make us question the origin of data and their validity for later analyses.
...
```{r}
# ordering rows in data before and after ComBat by batch order
ord <- order(pheno$batch)
edata_sorted <- edata[, ord]
combat_edata_sorted <- combat_edata[, ord]

pdf("golebiewski_problem2_before.pdf", width = 8, height = 7)
...
heatmap.2(edata_sorted,
          ...
          margins = c(12, 9),     # widens margins around plot
          col = my_palette,       # use on color palette defined above
          dendrogram = "row",     # cluster only rows
          Colv = FALSE,           # do not cluster columns (samples)
          scale = "row")
dev.off()
pdf("golebiewski_problem2_after.pdf", width = 8, height = 7)

heatmap.2(combat_edata_sorted,
          main = "Bladder Cancer Data Cleaned by ComBat (After Correction)", # heat map title
          notecol = "black",      # change font color of cell labels to black
          density.info = "none",  # turns off density plot inside color legend
          trace = "none",         # turns off trace lines inside the heatmap
          margins = c(12, 9),     # widens margins around plot
...
```{r}
# Pearson correlation and labels
cor <- cor(edata.log, method = "pearson")
cor_labels <- paste0(pheno[, 'cancer'], ', Batch ', pheno[, 'batch'])

pdf("golebiewski_problem3.pdf", width = 8, height = 8)
...
```{r}
bottomly_pData <- pData(bottomly.eset)
bottomly_exprs <- exprs(bottomly.eset)
bottomly_exprs <- bottomly_exprs[rowMeans(bottomly_exprs) > 10, ] # mean expression > 10
bottomly_exprs <- log2(bottomly_exprs + 1) # add 1 so no log(0) appears

mod_uncorrected <- lm(t(bottomly_exprs) ~ as.factor(bottomly_pData$strain) + as.factor(bottomly_pData$experiment.number))
mod_uncorrected_tidy <- tidy(mod_uncorrected)

combat_bottomly_exprs <- ComBat(dat = bottomly_exprs,
                                batch = bottomly_pData$experiment.number,
                                mod = model.matrix(~1, data = bottomly_pData),
                                par.prior = TRUE,
                                prior.plots = TRUE)
mod_combat <- lm(t(combat_bottomly_exprs) ~ as.factor(bottomly_pData$strain))
mod_combat_tidy <- tidy(mod_combat)

strain_term <- "as.factor(bottomly_pData$strain)DBA/2J"
est_compare <- tibble(
  LinearModel = mod_uncorrected_tidy %>% filter(term == strain_term) %>% pull(estimate),
  ComBat = mod_combat_tidy %>% filter(term == strain_term) %>% pull(estimate)
)


pdf("golebiewski_problem4_coeffs.pdf", width = 8, height = 7)
# scatter plot of coefficeints
ggplot(est_compare, aes(x = LinearModel, y = ComBat)) + 
  geom_point(col = "darkgrey", alpha = 0.5, size = 0.5) + geom_abline(slope = 1, intercept = 0, col = "darkred") +
  geom_smooth(method = "lm", se = TRUE) + theme_bw() + labs(title = "Strain effect estimates", x = "Uncorrected expression", y = "ComBat-corrected expression")
dev.off()

pdf("golebiewski_problem4_p-vals_uncorrected.pdf", width = 8, height = 7)
# p-val histogram from uncorrected
ggplot(mod_uncorrected_tidy %>% filter(term == strain_term)) + geom_histogram(aes(x = p.value), bins = 100, fill = "darkorange") + theme_bw() +labs(title = "Uncorrected expression p-values", x = "p-value")

dev.off()
pdf("golebiewski_problem4_p-vals_corrected.pdf", width = 8, height = 7)
# p-val histogram from combat corrected
ggplot(mod_combat_tidy %>% filter(term == strain_term)) + geom_histogram(aes(x = p.value), bins = 100, fill = "darkorange") + theme_bw() +labs(title = "ComBat-corrected expression p-values", x = "p-value")
dev.off()
```
...
```{r}
# Combat already prepared in hw4
# we can set the SVA
mod_strain <- model.matrix(~as.factor(bottomly_pData$strain))
mod0_strain <- model.matrix(~1, data = bottomly_pData)

sva_output <- sva(bottomly_exprs, mod = mod_strain, mod0 = mod0_strain, n.sv = num.sv(bottomly_exprs, mod_strain, method = "leek")) #leek method as in line 462

mod_sva <- lm(t(bottomly_exprs) ~ as.factor(bottomly_pData$strain) + sva_output$sv)
mod_sva_tidy <- tidy(mod_sva)

# and exctract the estimates
est_compare <- tibble(
  ComBat = mod_combat_tidy %>% filter(term == strain_term) %>% pull(estimate),
  SVA = mod_sva_tidy %>% filter(term == strain_term) %>% pull(estimate)
)

pdf("golebiewski_problem5_coeffs.pdf", width = 8, height = 7)
ggplot(est_compare, aes(x = ComBat, y = SVA)) +
  geom_point(col = "darkgrey", alpha = 0.5, size = 0.5) +
  geom_abline(intercept = 0, slope = 1, col = "darkred") + geom_smooth(method = "lm", se = TRUE) + theme_bw() +
  labs(title = "Strain effect estimates", x = "ComBat-corrected", y = "SVA-corrected")
dev.off()


# p-values
pvals_compare <- tibble(
  ComBat = mod_combat_tidy %>% filter(term == strain_term) %>% pull(p.value),
  SVA = mod_sva_tidy %>% filter(term == strain_term) %>% pull(p.value)
)

pdf("golebiewski_problem5_p-vals_combat.pdf", width = 8, height = 7)
ggplot(pvals_compare, aes(x = ComBat)) +
  geom_histogram(bins = 100, fill = "darkorange") + theme_bw() +
  labs(title = "ComBat-corrected p-values", x = "p-value")
dev.off()

pdf("golebiewski_problem5_p-vals_sva.pdf", width = 8, height = 7)
ggplot(pvals_compare, aes(x = SVA)) +
  geom_histogram(bins = 100, fill = "darkorange") + theme_bw() +
  labs(title = "SVA-corrected p-values", x = "p-value")
dev.off()
